...
function _download() {
  _download = _asyncToGenerator(function* (_ref) {
    ...
    if (name.length <= 1) name = "paper-".concat(version, "-").concat(relesa, ".jar");
...
function getLinkDownload(_x2, _x3) {
  return _getLinkDownload.apply(this, arguments);
}

function _getLinkDownload() {
  _getLinkDownload = _asyncToGenerator(function* (version, release) {
    return "https://papermc.io/api/v2/projects/paper/versions/".concat(version, "/builds/").concat(release, "/downloads/paper-").concat(version, "-").concat(release, ".jar");
  });
  return _getLinkDownload.apply(this, arguments);
}

function createFiles(_x4, _x5, _x6, _x7, _x8) {
  return _createFiles.apply(this, arguments);
}

function _createFiles() {
  _createFiles = _asyncToGenerator(function* (version, relesa, paths, name, promises) {
    var _loop = function* _loop(i) {
      var sym = _path.default.resolve(dirname, paths[i], name);

      var writer = yield createWriteStream(sym);
      var res = yield getStreamPaper(version, relesa);
...
function createWriteStream(_x9) {
...
function getStreamPaper(_x10, _x11) {
  return _getStreamPaper.apply(this, arguments);
}

function _getStreamPaper() {
  _getStreamPaper = _asyncToGenerator(function* (version, relesa) {
    var res = yield (0, _axios.default)({
      url: yield getLinkDownload(version, relesa),
      method: "GET",
